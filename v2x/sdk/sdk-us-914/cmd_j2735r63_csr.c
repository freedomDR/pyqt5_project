...
static int stop_csr = 0;

/***************************************************************/
 /***************************************************************/
 void *j2735_csr_calloc(size_t nmemb, size_t size)
 {
...
void stop_csr_func(ULONG input)
{
	(void)input;
	stop_csr = 1;
	return;
...
void _cmd_csr_free_struct(csr_standard_item *free_csr_item)
{
	if(free_csr_item == NULL){
		printf("[CSR] Error: free_csr_struct() input is NULL\n");
	}else{
		if(free_csr_item->requests != NULL){ 		
			j2735_csr_free(free_csr_item->requests);
		}
		j2735_csr_free(free_csr_item);
	}
}

void _show_csr_standard_item(csr_standard_item *decoded_item)
{
	int i;
	...
	/*(2) msgCnt, OP*/
	if(decoded_item->mask_msg_cnt == 1){
	...
	{
		printf("\nrequests[%d].requestedItem = %d\n", i, decoded_item->requests[i].requestedItem);
	}	
	printf("===============================\n");
}
...
int cmd_rxCsr(cli_instance_t *cli, const char *command, char *argv[], int argc)
{
	...
	csr_standard_item *decoded_item = NULL;
	int err_csr;

	int err, ret;
	TX_TIMER stop_csr_timer;
	unsigned int psid;
	wme_handle_t handle;
	...
	stop_csr = 0;
	tx_timer_create(&stop_csr_timer, NULL, stop_csr_func, 0, 5000, 1, TX_AUTO_ACTIVATE);
	while(stop_csr == 0) {
		...
		decoded_item = (csr_standard_item *)j2735_csr_calloc(1, sizeof(csr_standard_item));
		err_csr = j2735r63_csr_decode(decoded_item, in_wsm.data, MAX_CSR_BUFF_SIZE, DECODE_PRINT_LEVEL);
		if(err_csr < 0)
		{
			printf("decode error...\n");
			return -1;
		...
		if(SHOW_DECODED == 1){_show_csr_standard_item(decoded_item);}
    
		j2735r63_csr_free_struct(decoded_item);
	}
	...
	tx_timer_delete(&stop_csr_timer);
	wme_exit(&handle);
...
int cmd_j2735r63_csr(cli_instance_t *cli, const char *command, char *argv[], int argc)
{
	...
	csr_standard_item *encode_item = NULL;
	csr_standard_item *decoded_item = NULL;
	int err_csr, i, loopCnt;  
	size_t size;
	unsigned char *encodeBuf = NULL;

	printf("%s", "Start J2735R63 CSR test!\n");

	loopCnt = 0;
	while(loopCnt < MAX_LOOP_COUNT)
	{
		encodeBuf = (unsigned char *)j2735_csr_calloc(1, MAX_CSR_BUFF_SIZE);
		encode_item = (csr_standard_item *)j2735_csr_calloc(1, sizeof(csr_standard_item));
		/*(1) timeStamp, OPTIONAL*/
		encode_item->mask_time_stamp = 1;
		encode_item->time_stamp = 527040;		
		/*(2) msgCnt*/
		encode_item->mask_msg_cnt = 1;
		...
		encode_item->requests = (requested_item *)j2735_csr_calloc(encode_item->requests_num, sizeof(requested_item));
		if(!(encode_item->requests))
		{
			printf("[CSR] Error: MEMORY_ALLOCATE_FAIL\n");
			return -MEMORY_ALLOCATE_FAIL;		 
		}else
		{
			for(i = 0; i < encode_item->requests_num; i++)
			{
				encode_item->requests[i].requestedItem = 16;
			}			
		}
		encode_item->mask_regional = 0;		/*(1) msgCnt*/
		
		/********************************************************************/

		if((loopCnt % 100) == 0)	
			printf("CSR loopCnt = %d\n", loopCnt + 1);

		//if(SHOW_ENCODE == 1){_show_csr_standard_item(&encode_item);}

		err_csr = j2735r63_csr_encode(encode_item, encodeBuf, MAX_CSR_BUFF_SIZE, &size, ENCODE_PRINT_LEVEL);
		if(err_csr < 0)
		{
			printf("csr encode fail...\n");		
			return -1;	
		}
		_cmd_csr_free_struct(encode_item);

		decoded_item = (csr_standard_item *)j2735_csr_calloc(1, sizeof(csr_standard_item));
		err_csr = j2735r63_csr_decode(decoded_item, encodeBuf, MAX_CSR_BUFF_SIZE, ENCODE_PRINT_LEVEL);
		if(err_csr < 0)
		{
			printf("decode error...\n");
			return -1;
		...
		j2735r63_csr_free_struct(decoded_item);
		j2735_csr_free(encodeBuf);

		loopCnt++;
		(void) tx_thread_sleep(10);
	}
	
	printf("%s", "J2735R63 CSR Test command executed successfully.\n");
		
	return CLI_OK;	
}

int _encode_txCsr(unsigned char *encodeBuf, size_t input_buf_size, size_t *size)
{
	int i, err_csr;
	csr_standard_item *encode_item = NULL;
	...
	}else
	{
		for(i = 0; i < encode_item->requests_num; i++)
		{
			encode_item->requests[i].requestedItem = i;
		}			
	}
	...
	{
		printf("\nCSR Encode Data:\n");
		_show_csr_standard_item(encode_item);
	}
	err_csr = j2735r63_csr_encode(encode_item, encodeBuf, input_buf_size, size, ENCODE_PRINT_LEVEL);
	if(err_csr < 0)
	{				
		printf("csr encode fail...\n");				
		return -1;		
...
int cmd_txCsr(cli_instance_t *cli, const char *command, char *argv[], int argc)
{
	...
	if(argc != 6){ /* In threadx, cmd is not included in argc */
		printf("txCsr <psid> <data_rate> <tx_power> <dest_mac> <interval> <num>\n");
		printf("-- usage --\n");
		...
		printf("-- example --\n");
		printf("txCsr 123 6 18 FF:FF:FF:FF:FF:FF 1000 2\n");
		return 1;
	}
	...
	encodeBuf = (unsigned char *)j2735_csr_calloc(1, MAX_CSR_BUFF_SIZE);
	_encode_txCsr(encodeBuf, MAX_CSR_BUFF_SIZE, &len);
	...
	printf("%s", "cmd_txCsr command executed successfully.\n");
	return CLI_OK;
}
